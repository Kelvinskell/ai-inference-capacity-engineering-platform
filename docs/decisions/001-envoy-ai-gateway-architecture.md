...
- I use API-key-derived client identity with explicit `Exact` selectors so callers cannot select another client's quota bucket.
...
- I do not enforce a strict 1 million token maximum for the development client. It receives a preferred 1 million token allowance and can borrow unused capacity from the shared 100 million token pool.
